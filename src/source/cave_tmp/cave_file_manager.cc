...
void MatrixFileHandler::save(const std::vector<std::vector<bool>> &matrix, const std::string &filename) {
    std::ofstream fileoutstream(filename);
    if (!fileoutstream) {
        throw std::invalid_argument("File opening error");
    }
    fileoutstream << matrix.size() << " " << ((matrix.size() != 0) ? matrix[0].size() : 0)  << std::endl;
    for (auto &row : matrix) {
        for (auto cell : row) {
            fileoutstream << (cell ? 0 : 1) << " ";
        }
...
std::vector<std::vector<bool>> MatrixFileHandler::load(const std::string &filename) {
    size_t rows = 0;
    size_t cols = 0;
